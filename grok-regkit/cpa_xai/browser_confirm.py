    ...
    try:
        ...
        if reg_file.is_file():
            ...
            try:
                ...
                # Prefer headed under Xvfb on servers; pure headless often fails CDP + CF.
                opts = create_browser_options(force_headless=bool(headless))
                log(
                    "using register create_browser_options "
                    f"(force_headless={bool(headless)})"
                )
            except TypeError:
                try:
                    from grok_register_ttk import create_browser_options  # type: ignore
                    ...
                    log("using register create_browser_options (legacy signature)")
                except Exception as e:  # noqa: BLE001
...
def _detect_auth_error(text: str, url: str = "") -> str | None:
    ...
    needles = [
        ...
        ("error code 520", "cloudflare 520 origin error"),
        ("error code 521", "cloudflare 521 origin down"),
        ("error code 522", "cloudflare 522 connection timed out"),
        ("error code 523", "cloudflare 523 origin unreachable"),
        ("error code 524", "cloudflare 524 timeout"),
        ("web server is returning an unknown error", "cloudflare 520 origin error"),
        ("cloudflare.com for more information", "cloudflare error page"),
    ]
