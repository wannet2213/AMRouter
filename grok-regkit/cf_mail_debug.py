...
def generate_username(length: int = 10) -> str:
    """Generate a random mailbox name required by the cloudflare_temp_email admin API."""
    chars = string.ascii_lowercase + string.digits
    return "".join(secrets.choice(chars) for _ in range(length))


def normalize_path(path: str, default_path: str) -> str:
    """Normalize an API path, guarding against a missing leading slash."""
...
def build_auth_headers(auth_mode: str, api_key: str, content_type: bool = False) -> Dict[str, str]:
    """Build the Cloudflare temp-mail API auth headers from debug parameters."""
...
def create_address(
    ...
    """Create a Cloudflare temp-mail address, supporting both the anonymous and admin APIs."""
    ...
    if not data:
        raise RuntimeError(f"{path} is not JSON: {raw}")
